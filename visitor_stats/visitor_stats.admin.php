...
$filter_ip      = cot_import('ip', 'G', 'TXT', '');
$filter_ip      = (string) $filter_ip;
$filter_country = cot_import('country', 'G', 'TXT', '');
$filter_country = (string) $filter_country;
$limit = 50;
...
if (!empty($filter_ip)) {
    $condition .= ' AND vs_ip LIKE ?';
    $params[] = '%' . $filter_ip . '%';
}
if (!empty($filter_country)) {
    $condition .= ' AND vs_country LIKE ?';
...
$tt->assign([
    ...
    'VAL_FILTER_IP'      => htmlspecialchars($filter_ip),
    'VAL_FILTER_COUNTRY' => htmlspecialchars($filter_country),
    'VAL_PAGE'           => $pg,
